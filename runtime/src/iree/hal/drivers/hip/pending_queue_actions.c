...
// Count actions or things with count O(number-of-action) like work items.
// We assume the number of pending actions will be small enough so we don't
// resort to iree_host_size_t.
typedef int32_t action_count_t;
...
static const action_count_t initial_work_items_per_action = 2;
...
static void iree_hal_hip_atomic_slist_entry_push_front(
    iree_hal_hip_atomic_slist_entry_t* list,
    iree_hal_hip_queue_action_t* action) {
  IREE_ASSERT(!action->next && !action->prev);

  iree_hal_hip_queue_action_t* head = list->ready_list_head;
  list->ready_list_head = action;
  if (head) {
    action->next = head;
    head->prev = action;
  }
...
static_assert(sizeof(iree_status_t) <= sizeof(iree_atomic_intptr_t),
              "iree_status_t must be able to store in iree_atomic_intptr_t.");
...
struct iree_hal_hip_pending_queue_actions_t {
  ...
  // pending work items.
  action_count_t pending_work_items_count IREE_GUARDED_BY(action_mutex);
...
// Notify work and completion threads that there is work.
...
iree_status_t iree_hal_hip_pending_queue_actions_enqueue_execution(
  ...
  if (iree_status_is_ok(status)) {
    ...
    if (actions->exit_requested) {
      ...
      actions->pending_work_items_count += initial_work_items_per_action;
    }
    iree_slim_mutex_unlock(&actions->action_mutex);
  } else {
    iree_hal_resource_set_free(action->resource_set);
...
// Fail and destroy the action.
// Does not consume |status|.
// Decrement pending work items count accordingly based on the unfulfilled
...
static void iree_hal_hip_queue_action_fail_locked(
  ...
  action_count_t work_items_remaining;
  switch (action->state) {
    case IREE_HAL_HIP_QUEUE_ACTION_STATE_ALIVE:
      work_items_remaining = initial_work_items_per_action;
      break;
...
// Fail and destroy all actions.
// Does not consume |status|.
static void iree_hal_hip_ready_action_slist_fail_locked(
    iree_hal_hip_ready_action_slist_t* list, iree_status_t status) {
  iree_hal_hip_atomic_slist_entry_t* entry =
      iree_hal_hip_ready_action_slist_pop(list);
  while (entry) {
...
// Fail and destroy all actions and set status.
// Does not consume |status|.
...
static void iree_hal_hip_pending_queue_actions_fail_locked(
  ...
  iree_hal_hip_ready_action_slist_fail_locked(
...
static iree_status_t iree_hal_hip_pending_queue_actions_issue_execution(
  ...
  iree_hal_hip_completion_slist_push(&actions->completion_area.completion_list,
                                     entry);
...
static void iree_hal_hip_worker_process_ready_list(
  ...
  while (true) {
    iree_hal_hip_atomic_slist_entry_t* entry =
        iree_hal_hip_ready_action_slist_pop(
    ...
    while (entry->ready_list_head) {
      ...
      if (IREE_UNLIKELY(!iree_status_is_ok(status))) {
        iree_hal_hip_atomic_slist_entry_push_front(entry, action);
        iree_hal_hip_ready_action_slist_push(
